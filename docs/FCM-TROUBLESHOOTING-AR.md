...
### خطأ `UNREGISTERED` / `NotRegistered` في اللوج

يعني التوكن المخزّن في `fcm_tokens` **لم يعد مقبولاً عند Firebase** (تطبيق أعيد تثبيته، مسح بيانات، توكن قديم، أو عدم تطابق مشروع نادر).

- **استخدم `USER_ID` الصحيح** في الاختبار: نفس `id` لحسابك في `users` (مثلاً admin غالباً `1`) وليس أي رقم عشوائي.
- بعد تحديث الـ backend: عند **حفظ توكن جديد** يُحذف التوكن القديم لنفس المنصة، وعند إرسال فاشل بـ UNREGISTERED يُحذف التوكن تلقائياً من الجدول.
- ثم: **افتح التطبيق** بحسابك → **سجّل دخول** → اسمح بالإشعارات → أعد:
  `docker exec easytech_api_v2 php /var/www/html/backend/fcm_send_test_cli.php 1`
